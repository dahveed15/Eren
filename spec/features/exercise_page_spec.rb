...
RSpec.describe 'Add Activity page', type: :feature, js: true do
  ...
  context 'with a previous activity already created' do
    ...
    scenario 'index page' do
      log_in(user)
      visit new_activity_exercise_path(benchpress_activity)
      ...
      expect(benchpress_activity.exercises.count).to eq(1)
      expect(page).to have_content('3 sets of 10 reps at 120 kg')

    end

    context 'with an exercise already created' do
      let!(:previous_exercise) { benchpress_activity.exercises.create(reps: 12, sets: 5, weight_value: 100, units: 'kg')}
